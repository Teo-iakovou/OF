...
async function POST(req) {
    ...
    try {
        const r = await fetch(`${SERVER_BASE_URL}/api/auth/logout`, {
            method: "POST",
            headers
        });
        try {
            if ("TURBOPACK compile-time truthy", 1) console.log('[auth-bff] POST /api/auth/logout', {
                hadCookie: !!token,
                status: r.status
            });
        } catch  {}
        // Pass through backend Set-Cookie so the backend remains the single cookie owner.
        const setCookieList = r.headers.getSetCookie?.() ?? (r.headers.get("set-cookie") ? [
            r.headers.get("set-cookie")
        ] : []);
        const success = r.ok && setCookieList.length > 0;
        const res = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(success ? {
            ok: true
        } : {
            ok: false,
            error: "LOGOUT_FAILED"
