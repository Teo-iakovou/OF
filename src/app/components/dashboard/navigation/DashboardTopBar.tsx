  ...
  return (
    <div className={`md:hidden fixed left-0 right-0 ${DASHBOARD_LAYOUT.mobileTopBarTopOffset} z-40 flex justify-center`}>
      <div className="w-full max-w-6xl bg-[color:color-mix(in_oklab,white_3%,transparent)] backdrop-blur-[2px]">
        <div className="mx-auto relative flex h-[calc(env(safe-area-inset-top,0px)+2.75rem)] w-full items-center justify-between px-4">

          {/* Left: Settings */}
          <button
            aria-label={t("openSettingsLabel")}
            onClick={onOpenSettings}
            className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-white/90 transition hover:bg-white/10 hover:text-white"
          >
            <Settings size={16} />
          </button>
