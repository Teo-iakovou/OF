...
function CoachChat({
  ...
  const catLabels: Record<PromptCategory, string> = {
    ...
    "Captions": t("catCaptions"),
    "Hooks": t("catHooks"),
    "Avatar / Video": t("catAvatarVideo"),
    "Planning": t("catPlanning"),
  };
  const catPrompts: Record<PromptCategory, string[]> = {
    "Strategy": t.raw("promptsStrategy") as string[],
    "Captions": t.raw("promptsCaptions") as string[],
    "Hooks": t.raw("promptsHooks") as string[],
    "Avatar / Video": t.raw("promptsAvatarVideo") as string[],
    "Planning": t.raw("promptsPlanning") as string[],
  };
...
function tokenizeWordsWithSpaces(s: string) {
    ...
    try {
      ...
      switch (res.status) {
        case 402: {
          ...
          setError(data.error || t("errorChatLimit"));
          setIsSending(false);
        ...
        case 429: {
          ...
          setError(data.message || data.error || t("errorRateLimit"));
          setIsSending(false);
          if (!conversation?._id) setConversation(null);
          return;
        }
  ...
  return;
        ...
        default: {
          const data = res.data as CoachChatGenericError;
          setError(data.error || data.message || t("errorChatFailed"));
          setIsSending(false);
          if (!conversation?._id) setConversation(null);
          return;
        }
  ...
  return (
    <button
      ...
      aria-label={copied ? t("copied") : t("copyAriaLabel")}
  ...
  function exportConversation() {
    ...
    msgs.forEach((msg) => {
      if (msg.role === "user") {
        lines.push(`${t("exportYou")}: ${msg.content.trim()}`);
        lines.push("");
      } else if (msg.role === "assistant") {
        lines.push(`${t("exportSage")}: ${stripMarkdown(msg.content)}`);
        lines.push("");
        lines.push("---");
        lines.push("");
      }
...
>
  {bootstrapping ? (
    ...
    <p className="text-gray-500 text-center italic">{t("emptyState")}</p>
  ) : (
    <>
  {(() => {
                ...
                {(["up", "down"] as const).map((vote) => {
                  ...
                  return (
                    <button
                      ...
                      aria-label={vote === "up" ? t("feedbackHelpful") : t("feedbackNotHelpful")}
                    >
                      {vote === "up"
      ...
      >
        {isContextLimited ? (
          <div className="mb-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-3 text-sm text-amber-100">
            <div className="font-semibold">{t("contextLimitHeading")}</div>
            <div className="mt-1 text-amber-200">
              {t("contextLimitBody")}
            </div>
            {contextInfo?.limit ? (
              <div className="mt-1 text-xs text-amber-200">
                {t("contextLimitTokens", { used: Number(contextInfo.used ?? 0).toLocaleString(), limit: Number(contextInfo.limit).toLocaleString() })}
              </div>
            ) : null}
        ...
        {!isContextLimited && contextNearLimit ? (
          <div className="mb-3 rounded-xl border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-xs text-amber-100">
            <span className="font-semibold">{t("nearLimitHeading")}</span>{" "}
            {t("nearLimitBody")}
            <button
              type="button"
              onClick={handleStartNewChat}
        ...
        {isLimited && (
          <div className="mb-3 rounded-xl border border-[var(--hg-accent)]/30 bg-[var(--hg-accent)]/10 px-3 py-2 text-xs text-[var(--hg-accent)]">
            <span className="font-semibold">{t("monthlyLimitHeading")}</span>
            {" — "}
            <button
              type="button"
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-gray-400 mb-3">
                <span><span className="text-[var(--hg-accent)]">✦</span> {t("upgradeFeature1")}</span>
                <span><span className="text-[var(--hg-accent)]">✦</span> {t("upgradeFeature2")}</span>
                <span><span className="text-[var(--hg-accent)]">✦</span> {t("upgradeFeature3")}</span>
                <span><span className="text-[var(--hg-accent)]">✦</span> {t("upgradeFeature4")}</span>
              </div>
              <a
                href={PACKAGES_URL}
        ...
        {suggestionsOpen ? (
          <div className="mb-3 rounded-xl border border-[var(--hg-border)] bg-[var(--hg-surface)]/90 p-3">
            ...
            <QuickPromptsBar
              prompts={catPrompts[activeSuggestionCategory]}
              category={catLabels[activeSuggestionCategory]}
              label={t("suggestionsPickPrompt")}
        ...
        <div className="flex items-center gap-2 bg-[#1a1f2b] border border-gray-700 rounded-xl px-3 py-2">
          <button
            ...
            aria-label={t("suggestionsToggleAriaLabel")}
          ...
          <span className="relative group hidden sm:inline-flex items-center gap-1 text-[11px] text-gray-500 shrink-0 cursor-default select-none">
              ...
              <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max rounded-lg border border-gray-700 bg-gray-900 px-2.5 py-1.5 text-[11px] text-gray-200 opacity-0 group-hover:opacity-100 transition-opacity duration-75 whitespace-nowrap z-50 shadow-lg">
                {hasInputBarTokenData
                  ? t("tokensUsage", { used: fmtTokens(inputBarTokensUsed!), limit: fmtTokens(inputBarTokensLimit!) })
                  : t("tokensLoading")}
              </span>
          </span>
