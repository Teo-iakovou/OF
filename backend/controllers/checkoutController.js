...
const createCheckoutSession = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: withLocalePath(safeOrigin, locale, "/dashboard"),
    });
...
const createAddonCheckoutSession = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata,
      client_reference_id: email,
      success_url: `${withLocalePath(safeOrigin, locale, "/dashboard")}?status=success&kind=addon&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${withLocalePath(safeOrigin, locale, "/dashboard")}?status=cancel&kind=addon`,
    });
